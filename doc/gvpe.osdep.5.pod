...
=head2 Interface Initialisation

Unless otherwise notes, the network interface will be set "UP" with the
correct MAC address and correct MTU value. With most interface drivers,
...
approriate on the other side of the socket.  GVPE will exit if the MAC
