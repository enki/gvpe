...
(which is true for most routers, WLAN gateways and nameservers).

Finetuning needs to be done by editing C<src/vpn_dns.C> directly.
...
and lightweight (no RSA operations required). A host that receives ping
...
=item If the two hosts should be able to reach each other directly (common
...
are not known (such as dialup hosts), one side will send a I<mediated>
