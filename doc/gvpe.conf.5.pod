...
The DNS domain suffix that points to the DNS tunnel server. Needs to be
...
The port to bind the DNS tunnel socket to. Must be C<0> on all DNS tunnel
clients and C<53> on the server.
